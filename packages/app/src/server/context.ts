...
export const createContext = async ({ req, res }: CreateNextContextOptions) => {
	...
	const { data, error } = await tryCatch(prismaClient.user.findUnique({ where: { id: BigInt(user?.id ?? '0') } }));
	if (error) throw Error(error.message);
	if (!data) throw Error('Invalid User');
	return { req, res, user: data };
};

export type Context = { req: NextApiRequest; res: NextApiResponse; user: User | null };
